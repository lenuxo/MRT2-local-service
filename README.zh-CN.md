...
两个模型的硬件差异主要影响生成速度能否跟上播放速度。它们都可以在 Apple Silicon 上进行离线生成；下表中的“仅离线”表示官方基准未将该模型/设备组合列为实时，并不代表模型无法运行。
...
| M2 Max | 实时 | 实时 |
| M4 Pro | 实时 | 实时 |
| M2 Pro / M1 Pro | 实时 | 仅离线 |
| M4 Air / M3 Air / M1 Air | 实时 | 仅离线 |

Air 和多数 Pro 设备建议默认使用 `mrt2_small`。需要更高质量且可以接受生成慢于播放速度时，可以使用 `mrt2_base`；官方列出的 M4 Pro 与 Max 设备支持 Base 实时生成。官方表格未列出的设备没有明确的实时保证，详见[模型与推理参数](docs/MODELS.md)。
