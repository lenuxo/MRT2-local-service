...
文本和参考音频最终进入完整或流式核心命令，可以单独使用或按归一化权重混合。音频文件解码位于共享媒体层，Magenta 适配器只负责把核心 `AudioInput` 转为官方 `Waveform` 并融合两个 embedding；CLI、multipart HTTP 和 WebSocket 不直接依赖 Magenta 类型。
